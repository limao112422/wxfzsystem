  ...
  <div class="row">
              ...
              <div class="col-xs-12" style="background: #ffffff; padding-top:80px;height:170px;  ">
                ...
                <div class="col-xs-9" style="line-height: 20px;">
                  ...
                  <p style="color: #3399FF;"><?php echo ($f["member"]); ?><span style="color:#ccc;">人已发卡</span></p>
				  <?php  if(isset($_SESSION[C('USER_AUTH_KEY')])){ ?>
				  <p style="color: #3399FF;"><span style="color:#ccc;">佣金</span>&nbsp;￥<?php echo ($f["pay"]); ?>&nbsp;&nbsp;<span style="font-size:10px;background:#EE9C4D;padding:3px 6px;color:#FFFFFF;line-height:20px;border-radius:5px;"><?php echo ($f["jiesuan"]); ?></span></p>
				  <?php } ?>
		   ...
		   <?php  if(isset($_SESSION[C('USER_AUTH_KEY')])){ ?>
		   <div style="height:30px;line-height:30px;color:#EE9C4D;font-size:12px;">&nbsp;二级佣金：￥<?php echo ($f["eryong"]); ?>&nbsp;，&nbsp;三级佣金￥<?php echo ($f["sanyong"]); ?></div>
		    <?php } ?>
            基本信息
    ...
    <div class="col-xs-12" style=" padding-top:10px; background:#ffffff; margin-bottom:100%; ">
          <a href="">
...
<?php  }else{ ?>
	<div class="ssbootl" onclick="window.location.href='<?php echo U('/Register/tui',array('chanid'=>$f['id'],'style'=>1));?>'">推广海报</div>
	<button class="ssbootr" data-clipboard-text="<?php echo ($url); ?>" style="">复制链接</button>
<?php } ?>
...
function tanzi2(){
	...
	layui.use('layer', function(){ //独立版的layer无需执行这一句
      ...
      layer.open({
        ...
        ,btnAlign: 'c'
        ,moveType: 1 //拖拽模式，0或者1
        ,content: '<div style="padding: 20px; line-height: 22px;border-radius:5px; background-color: #FFF; color: #333; font-weight: 300;" id="timg"></div>'
        ,yes: function(layero){
